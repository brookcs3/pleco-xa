...
export class SpatialAudio {
  ...
  createDopplerNode() {
    ...
    pitchShifter.onaudioprocess = (e) => {
      ...
      const currentPitchFactor = lastPitchFactor * smoothFactor + pitchFactor * (1 - smoothFactor);
  ...
  updateScrollPosition(scrollProgress, scrollVelocity) {
    ...
    this.doppler.scrollVelocity = scrollVelocity || 
      (listenerY - this.doppler.lastScrollY) / deltaTime;
  ...
  updateSpatialNode(spatialNode, loopId, scrollProgress) {
    const { panner, dopplerShifter, distanceFilter, reverb, position } = spatialNode;
    ...
    const maxFreq = 20000;
    const minFreq = 2000;
    const filterFreq = maxFreq - (distance / this.space.height) * (maxFreq - minFreq);
    distanceFilter.frequency.linearRampToValueAtTime(filterFreq, this.context.currentTime + 0.1);
    
    // Update reverb mix based on distance
    const reverbAmount = Math.min(distance / this.space.height, 1) * 0.3;
    reverb.wetGain.gain.linearRampToValueAtTime(reverbAmount, this.context.currentTime + 0.1);
    reverb.dryGain.gain.linearRampToValueAtTime(1 - reverbAmount, this.context.currentTime + 0.1);
  ...
  calculateDopplerFactor(sourceY, listenerY) {
    ...
    const approachingSource = (sourceY > listenerY && relativeVelocity > 0) ||
  ...
  calculateScrollVelocity(currentScroll, previousScroll, deltaTime) {
    ...
    const metersPerSecond = (pixelsPerSecond / window.innerHeight) * this.space.height;
    
    // Smooth the velocity
    const smoothing = 0.8;
    return this.doppler.scrollVelocity * smoothing + metersPerSecond * (1 - smoothing);
  }
