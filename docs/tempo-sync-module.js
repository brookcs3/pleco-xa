...
export class TempoSync {
  ...
  async analyzeBuffer(audioBuffer) {
    ...
    const onsetEnvelope = await this.extractOnsetEnvelope(
      channelData,
      sampleRate,
    )
    ...
    // Find loop points
    const loopPoints = this.findOptimalLoopPoints(
      channelData,
      beats,
      sampleRate,
    )
  ...
  async extractOnsetEnvelope(channelData, sampleRate) {
    const frameCount = Math.floor(
      (channelData.length - this.fftSize) / this.hopSize,
    )
    ...
    for (let i = 0; i < frameCount; i++) {
      ...
      const frameData = channelData.slice(
        startSample,
        startSample + this.fftSize,
      )
  ...
  refineTempo(initialTempo, envelope, frameRate) {
    ...
    for (
      let tempo = initialTempo - tempoRange;
      tempo <= initialTempo + tempoRange;
      tempo += tempoStep
    ) {
  ...
  smoothEnvelope(envelope, windowSize = 5) {
    ...
    for (let i = 0; i < envelope.length; i++) {
      ...
      for (
        ...
        j <= Math.min(envelope.length - 1, i + halfWindow);
        j++
      ) {
        sum += envelope[j]
        count++
  ...
  syncLoops(masterTempo, slaveTempo, slaveSource) {
    ...
    if (slaveSource.playbackRate) {
      ...
      slaveSource.playbackRate.setValueAtTime(
        currentRate,
        this.context.currentTime,
      )
      slaveSource.playbackRate.linearRampToValueAtTime(
        targetRate,
  ...
  phaseAlign(masterBeats, slaveBeats, slaveSource, currentTime) {
    ...
    if (slaveSource.detune) {
      ...
      slaveSource.detune.linearRampToValueAtTime(
        0,
