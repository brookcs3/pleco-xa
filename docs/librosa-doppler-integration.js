...
export class XADopplerScroll extends EnhancedDopplerScroll {
  ...
  async musicalLoopAnalysis(audioBuffer, loopId) {
    ...
    debugLog(
      `${loopId} - Detected BPM: ${bpmData.bpm.toFixed(2)}, Bar duration: ${barDuration.toFixed(3)}s`,
    )
  ...
  async findMusicalLoopPoints(audioBuffer, bpmData) {
    ...
    for (const loopLength of musicalDivisions) {
      ...
      const segment2 = this.applyHannWindow(
        channelData.slice(loopSamples, loopSamples * 2),
      )
  ...
  setTransitionState(progress) {
    ...
    const windowSize = this.calculateMorphWindow(
      transitionProgress,
      loop1Confidence,
      loop2Confidence,
    )
    ...
    const confidenceRatio =
      loop2Confidence / (loop1Confidence + loop2Confidence)
    const adjustedProgress =
      this.easeInOutCubic(transitionProgress) * confidenceRatio +
    ...
    const loop1HighpassFreq =
      20 + transitionProgress * Math.min(1000, loop1Centroid * 0.2)
    const loop2LowpassFreq =
      Math.max(loop2Centroid * 2, 20000) - transitionProgress * 15000
    ...
    const rmsRatio =
      (this.tempoData.loop1.rms || 0.1) / (this.tempoData.loop2.rms || 0.1)
    ...
    if (
      transitionProgress >= switchThreshold &&
  ...
  applyWindowedTempoMorph(progress, windowSize) {
    ...
    const windowedProgress = this.applyHannWindowToProgress(
      progress,
      windowSize,
    )
    ...
    if (
      this.loops[slaveLoop].source &&
      this.loops[slaveLoop].source.playbackRate
    ) {
  ...
  computeSpectralCentroid(audioBuffer) {
    ...
    for (
      let i = 0;
      i < channelData.length - fftSize;
      i += this.analysisOptions.hopSize
    ) {
