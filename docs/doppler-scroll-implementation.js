...
export class DopplerScroll {
    ...
    pitchShift.onaudioprocess = (e) => {
      if (!this.state.isTransitioning) {
        e.outputBuffer.copyToChannel(
          e.inputBuffer.getChannelData(0), 0
        );
        return;
      }
  ...
  setPage1State() {
    ...
    this.loops.loop1.gain.gain.linearRampToValueAtTime(1, this.context.currentTime + 0.1);
    this.loops.loop1.filter.frequency.linearRampToValueAtTime(20, this.context.currentTime + 0.1);
    
    // Loop 2: Silent
    this.loops.loop2.gain.gain.linearRampToValueAtTime(0, this.context.currentTime + 0.1);
  }
  ...
  setPage3State() {
    ...
    // Loop 1: Silent
    this.loops.loop1.gain.gain.linearRampToValueAtTime(0, this.context.currentTime + 0.1);
    
    // Loop 2: Full spectrum, 100% volume
    this.loops.loop2.gain.gain.linearRampToValueAtTime(1, this.context.currentTime + 0.1);
    this.loops.loop2.filter.frequency.linearRampToValueAtTime(20000, this.context.currentTime + 0.1);
  }
