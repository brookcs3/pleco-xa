...
import { loadAudioBuffer, computeRMS, computePeak, computeSpectrum } from '../../WIP/frontend/librosaLite.js';
...
export class EnhancedDopplerScroll extends DopplerScroll {
  ...
  async analyzeLoopsWithLibrosa() {
    ...
    this.tempoData.loop1 = {
      ...
      spectrum: await computeSpectrum(this.loops.loop1.buffer, this.analysisOptions.fftSize)
    };
    ...
    this.tempoData.loop2 = {
      ...
      spectrum: await computeSpectrum(this.loops.loop2.buffer, this.analysisOptions.fftSize)
    };
  }
  ...
  async analyzeAudioBuffer(audioBuffer, loopId) {
    ...
    const onsetEnvelope = await this.extractOnsetEnvelopeEnhanced(channelData, sampleRate);
    ...
    const loopPoints = await this.findLoopPointsEnhanced(channelData, sampleRate);
  ...
  async extractOnsetEnvelopeEnhanced(channelData, sampleRate) {
    const frameCount = Math.floor((channelData.length - this.analysisOptions.fftSize) / this.analysisOptions.hopSize);
   ...
   */
  computeMelSpectrogram(samples, sr, nMels = 40, frameSize = 2048, hopSize = 512) {
    const mel = Array(nMels).fill(0).map(() => []);
  ...
  detectTempoEnhanced(envelope, sampleRate) {
    ...
    for (let i = 1; i < autocorr.length - 1; i++) {
      if (autocorr[i] > autocorr[i-1] && autocorr[i] > autocorr[i+1] && autocorr[i] > maxCorr) {
        maxCorr = autocorr[i];
  ...
  extractBeatsEnhanced(channelData, tempo, sampleRate) {
    ...
    while (currentBeat < channelData.length) {
      ...
      const expectedNext = currentBeat + (beatPeriod * this.analysisOptions.hopSize);
      const searchStart = Math.max(0, expectedNext - beatPeriod * 0.1);
      const searchEnd = Math.min(channelData.length, expectedNext + beatPeriod * 0.1);
      ...
      currentBeat = bestPosition + (beatPeriod * this.analysisOptions.hopSize * 0.5);
    }
  ...
  async findLoopPointsEnhanced(channelData, sampleRate) {
    ...
    const endSlice = this.applyHannWindow(channelData.subarray(totalSamples - window));
    ...
    const endIndex = this.findZeroCrossing(channelData, totalSamples - window + bestOffset);
  ...
  computeSpectralCentroid(channelData, sampleRate) {
    const spectrum = this.dft(this.applyHannWindow(
