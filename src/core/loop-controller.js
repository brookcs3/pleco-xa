...
export class LoopController {
  ...
  setMinLoopDuration(durationSeconds) {
    ...
    console.log(`Loop controller: Min duration set to ${durationSeconds * 1000}ms`);
  }
  ...
  halfLoop() {
    ...
    if (this.audioBuffer && newDuration * this.audioBuffer.duration < this.minLoopDuration) {
      return {
        success: false,
    ...
    return {
      success: true,
      loop: { ...this.currentLoop },
      actualDurationMs: this.audioBuffer ? newDuration * this.audioBuffer.duration * 1000 : null
    };
  }
  ...
  setLoop(start, end) {
    ...
    if (this.audioBuffer && duration * this.audioBuffer.duration < this.minLoopDuration) {
      return {
        success: false,
        loop: this.currentLoop,
        reason: `Loop would be smaller than minimum ${this.minLoopDuration * 1000}ms`
