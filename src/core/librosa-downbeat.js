...
export function findDownbeatPhase(audioData, beats, tempo, sampleRate) {
  ...
  console.log(`🎯 Downbeat phase scores: [${phaseScores.map(s => s.toFixed(2)).join(', ')}]`);
  console.log(`🎯 Best phase: ${bestPhase} (beat ${bestPhase + 1} is likely the downbeat)`);
...
export function findFirstDownbeat(audioData, tempo, sampleRate) {
  ...
  const onsetResult = onsetDetect(audioData, sampleRate, { hopLength: 256, delta: 0.1 });
  const onsets = onsetResult.onsetTimes;
 ...
 */
export function findMusicalLoop(audioData, sampleRate, tempo, { 
  preferredBars = 4,
  minBars = 2,
  maxBars = 8 
} = {}) {
  ...
  const onsetResult = onsetDetect(audioData, sampleRate, { hopLength: 512, delta: 0.1 });
  const strongOnsets = onsetResult.onsetTimes.filter(onset => onset > 1.0 && onset < duration - 2.0);
  ...
  for (const numBars of barLengths) {
    ...
    for (const onset of strongOnsets) {
      ...
      console.log(`Testing ${numBars}-bar loop at ${loopStart.toFixed(2)}s: score=${score.toFixed(4)}`);
    ...
    for (let barNum = 0; barNum < Math.min(8, duration / barDuration - numBars); barNum++) {
