...
export function getWaveformPeaks(audioBuffer, options = {}) {
  ...
  if (opts.precision !== null) {
    data = data.map(val => Math.round(val * Math.pow(10, opts.precision)) / Math.pow(10, opts.precision));
  }
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    switch (opts.type) {
      case 'peaks':
        ...
        break;
      case 'rms':
        amplitude = Math.sqrt(segment.reduce((sum, val) => sum + val * val, 0) / segment.length);
        break;
      case 'average':
        amplitude = segment.reduce((sum, val) => sum + Math.abs(val), 0) / segment.length;
        break;
      default:
        amplitude = Math.max(...segment.map(Math.abs));
    }
 ...
 * ```
 */
export function getWaveformRange(audioBuffer, startTime, endTime, options = {}) {
  const sampleRate = audioBuffer.sampleRate;
  ...
  const rangeData = channelData.slice(startSample, Math.min(endSample, channelData.length));
...
export function analyzeWaveform(audioBuffer, options = {}) {
  ...
  const sortedData = Array.from(channelData).map(Math.abs).sort((a, b) => b - a);
...
function getChannelData(audioBuffer, channel) {
  if (channel === -1) {
    ...
    return audioBuffer.getChannelData(Math.min(channel, audioBuffer.numberOfChannels - 1));
  }
}
