...
export async function analyzeTempoVariations(audioBuffer, options = {}) {
  ...
  for (let start = 0; start + windowSamples < audioBuffer.length; start += hopSamples) {
    // Create sub-buffer for this window
    const windowData = audioBuffer.getChannelData(0).slice(start, start + windowSamples);
    ...
    const result = await detectBPM(tempBuffer, { ...options, useOnsetStrength: false });
  ...
  return {
    bpmOverTime,
    timePoints,
    averageBPM: bpmOverTime.reduce((sum, bpm) => sum + bpm, 0) / bpmOverTime.length,
