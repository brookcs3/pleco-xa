 ...
 */
export function tempo(y, sr = 22050, hop_length = 512, ac_size = 8.0, max_tempo = 320.0, 
                     min_tempo = 30.0, prior = true) {
    // Get onset strength
    ...
    const candidates = find_tempo_candidates(tempogram, sr, hop_length, max_tempo, min_tempo);
 ...
 */
export function find_tempo_candidates(tempogram, sr, hop_length, max_tempo, min_tempo) {
    ...
    const search_range = tempogram.slice(min_period, Math.min(max_period, tempogram.length));
...
function analyze_subdivisions(beat_times) {
    ...
    for (let i = 0; i < intervals.length - 1; i += 2) {
        if (i + 1 < intervals.length) {
            ...
            if (ratio > 1.2 && ratio < 2.0) { // Typical swing ratios
                swing_evidence++;
            }
        }
    }
