 ...
 */
export function chroma_cqt(y, sr = 22050, hop_length = 512, fmin = null, n_chroma = 12,
                    tuning = 0.0, n_octaves = 7, bins_per_octave = 12) {
    
    // Compute CQT
    const cqt = constant_q_transform(y, sr, hop_length, fmin, n_octaves * bins_per_octave, tuning);
 ...
 */
export function chroma_stft(y, sr = 22050, hop_length = 512, n_fft = 2048, n_chroma = 12, tuning = 0.0) {
    // Compute STFT
 ...
 */
export function constant_q_transform(y, sr, hop_length, fmin = null, n_bins = 84, tuning = 0.0) {
    if (fmin === null) {
        fmin = 32.7; // C1
    }
...
export function cqt_to_chroma(cqt, n_chroma = 12, bins_per_octave = 12) {
    ...
    const chroma = Array(n_chroma).fill(null).map(() => new Float32Array(n_frames));
 ...
 */
export function stft_to_chroma(magnitude_spectra, sr, n_fft, n_chroma = 12, tuning = 0.0) {
    ...
    for (let t = 0; t < n_frames; t++) {
        for (let f = 0; f < magnitude_spectra[t].length; f++) {
            ...
            if (chroma_class >= 0 && chroma_class < n_chroma) {
                chroma[chroma_class][t] += magnitude_spectra[t][f] * magnitude_spectra[t][f];
            }
        }
...
export function enhance_chroma(chroma, norm = 2) {
    ...
    const enhanced = Array(n_chroma).fill(null).map(() => new Float32Array(n_frames));
