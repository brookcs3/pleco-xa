...
export class BeatTracker {
    ...
    beatTrack(options = {}) {
        ...
        console.log(`🥁 Beat tracking: ${tempo.toFixed ? tempo.toFixed(1) : 'dynamic'} BPM, ${beats.length} beats`);
    ...
    plp(options = {}) {
        ...
        if (tempoMin !== null && tempoMax !== null && tempoMax <= tempoMin) {
            throw new Error(`tempoMax=${tempoMax} must be larger than tempoMin=${tempoMin}`);
        }
        ...
        const ftmag = ftgram.map(frame => 
            frame.map(bin => Math.log1p(1e6 * Math.sqrt(bin.real * bin.real + bin.imag * bin.imag)))
        );
        ...
        for (let i = 0; i < ftgram.length; i++) {
            const maxMag = Math.max(...ftgram[i].map(bin => 
                Math.sqrt(bin.real * bin.real + bin.imag * bin.imag)
            ));
            for (let j = 0; j < ftgram[i].length; j++) {
                const mag = Math.sqrt(ftgram[i][j].real * ftgram[i][j].real + 
    ...
    _normalizeOnsets(onsets) {
        ...
        const variance = onsets.reduce((a, b) => a + Math.pow(b - mean, 2), 0) / (onsets.length - 1);
    ...
    _beatTrackDP(localScore, framesPerBeat, tightness) {
        ...
        for (let i = 1; i < N; i++) {
            ...
            for (let loc = searchStart; loc <= searchEnd; loc++) {
                ...
                const score = cumScore[loc] - 
                    tightness * Math.pow(logInterval - logFpb, 2);
    ...
    _trimBeats(localScore, beats) {
        ...
        const rms = Math.sqrt(beatScores.reduce((a, b) => a + b * b, 0) / beatScores.length);
    ...
    _computeMagnitudeSpectrum(frame) {
        ...
        return fft.map(bin => Math.sqrt(bin.real * bin.real + bin.imag * bin.imag));
    }
    ...
    _ifft(spectrum) {
        ...
        for (let n = 0; n < N; n++) {
            ...
            for (let k = 0; k < N; k++) {
                const angle = 2 * Math.PI * k * n / N;
                value += spectrum[k].real * Math.cos(angle) - 
    ...
    generateClickTrack(beats, duration, clickFreq = 880) {
        ...
        beats.forEach(beatTime => {
            ...
            for (let i = 0; i < clickSamples && startSample + i < samples; i++) {
                ...
                channelData[startSample + i] = 0.3 * envelope * Math.sin(2 * Math.PI * clickFreq * t);
            }
        });
