...
export async function load(url, {
  ...
  const end         = duration === null ? decoded.length
  ...
  if (sr !== null && sr !== nativeSr) y = resample(y, { origSr: nativeSr, targetSr: sr });
...
export function lpc(signal, order) {
  if (order < 1 || order >= signal.length) throw new RangeError('Invalid LPC order');
...
export function tone(frequency, { sr = 22050, length = null, duration = null, phi = -Math.PI * 0.5 } = {}) {
  if (length == null) {
...
export function chirp({ fmin, fmax, sr = 22050, length = null, duration = null, linear = false, phi = -Math.PI * 0.5 } = {}) {
  if (length == null) {
    if (duration == null) throw new Error('length or duration required');
    length = Math.floor(duration * sr);
  }
  ...
  for (let n = 0; n < length; ++n) {
    const t  = n / sr;
    const k  = linear ? (fmin + (fmax - fmin) * (t / T))
...
export function clicks({ times = null, frames = null, sr = 22050, hopLength = 512, clickFreq = 1000, clickDuration = 0.1, click = null, length = null } = {}) {
  if (!times && !frames) throw new Error('Provide "times" or "frames"');
  const positions = times ? times.map(t => Math.round(t * sr))
                          : frames.map(f => f * hopLength);
  if (!click) {
    const cLen  = Math.floor(sr * clickDuration);
    click       = new Float32Array(cLen);
    const w     = 2 * Math.PI * clickFreq / sr;
    for (let i = 0; i < cLen; ++i) click[i] = Math.pow(2, -10 * i / cLen) * Math.sin(w * i);
  }
...
export function muCompress(x, { mu = 255, quantize = true } = {}) {
  ...
  for (let i = 0; i < out.length; ++i) q[i] = Math.round((out[i] + 1) * mu / 2 - mu / 2);
  return q;
}
