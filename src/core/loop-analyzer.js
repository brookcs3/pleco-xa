...
export async function musicalLoopAnalysis(audioBuffer, bpmData) {
  ...
  for (const len of candidates) {
    ...
    debugLog(`Candidate length: ${len.toFixed(3)}s, Correlation: ${r.toFixed(3)}`);
  ...
  debugLog(`Candidates: ${results.map(r => `Start: ${r.loopStart}, End: ${r.loopEnd}, Confidence: ${r.confidence}`).join('; ')}`);
  if (results.length === 0) {
...
export async function fastOnsetLoopAnalysis(audioBuffer, bpmData = null) {
  ...
  try {
    ...
    debugLog(`Recurrence detection: ${res.loopStart.toFixed(3)}-${res.loopEnd.toFixed(3)} s`);
    return res;
  } catch (err) {
...
export async function xaLoopAnalysis(audioBuffer) {
  ...
  const rms = computeRMS(audioBuffer);
  const peak = computePeak(audioBuffer);
  const spectralCentroidVal = spectralCentroid({ y: audioData, sr: audioBuffer.sampleRate });
  const zeroCrossingRate = computeZeroCrossingRate(audioBuffer);

  console.log('Core stats computed:', { rms, peak, spectralCentroidVal, zeroCrossingRate });
