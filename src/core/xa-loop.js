...
export async function fastLoopAnalysis(audioBuffer) {
  ...
  console.log(`🎵 Detected ${beatResult.tempo.toFixed(1)} BPM with ${beatResult.beats.length} beats`);
  ...
  if (musicalLoop) {
    console.log(`🎵 Found ${musicalLoop.bars}-bar loop: ${musicalLoop.start.toFixed(3)}s - ${musicalLoop.end.toFixed(3)}s`);
    ...
    const normalizedConfidence = Math.min(1, Math.max(0, Math.abs(musicalLoop.score) * 1000));
    ...
    console.log(`🎯 Final loop: ${bestLoop.start.toFixed(3)}s - ${bestLoop.end.toFixed(3)}s`);
...
function findMainSection(audioData, onsetResult, sampleRate) {
  ...
  const meanEnergy = smoothedEnergies.reduce((sum, e) => sum + e, 0) / smoothedEnergies.length;
...
function findDownbeats(audioData, beatResult, sampleRate) {
  ...
  for (let i = 0; i < beatStrengths.length - 4; i++) {
    ...
    for (let j = 1; j <= 3; j++) {
      if (i + j < beatStrengths.length && beatStrengths[i + j].strength > current) {
        isStrongest = false;
        break;
      }
    }
 ...
 */
function findLoopCandidates(audioData, beatResult, onsetResult, sampleRate, mainSection = null, downbeats = []) {
  const beats = beatResult.beats;
  ...
  for (const numBeats of musicalDivisions) {
    ...
    if (downbeats.length > 0) {
      ...
      for (const downbeatTime of downbeats) {
        ...
        if (mainSection) {
          if (downbeatTime < mainSection.start || endTime > mainSection.end) continue;
        }
      ...
      for (let startBeatIdx = 0; startBeatIdx < beats.length - numBeats; startBeatIdx += Math.max(1, Math.floor(numBeats / 4))) {
        ...
        const actualEndTime = endBeatIdx < beats.length ? beats[endBeatIdx] : endTime;
 ...
 */
function analyzeLoopCandidate(audioData, startTime, endTime, numBeats, sampleRate, onsets, confidenceMultiplier = 1.0) {
  ...
  const nextSegmentEnd = Math.min(nextSegmentStart + loopLength, audioData.length);
  
  let correlation = 0;
  
  if (nextSegmentEnd - nextSegmentStart >= loopLength * 0.8) {
    // We have enough audio to compare - test if it loops well
    const nextSegment = audioData.slice(nextSegmentStart, nextSegmentStart + loopLength);
  ...
  for (const onset of onsets) {
    if (Math.abs(onset - startTime) < tolerance || Math.abs(onset - endTime) < tolerance) {
      onsetBonus += 0.1;
    }
  }
  ...
  let confidence = (normalizedCorrelation * 0.7 + onsetBonus * 0.3) * confidenceMultiplier;
  
  // Prefer certain musical divisions
  const musicalDivision = numBeats / 4;
  if (musicalDivision === 2 || musicalDivision === 4) confidence *= 1.3; // 2 or 4 bars (most common)
  else if (musicalDivision === 1) confidence *= 1.2; // 1 bar
  else if (musicalDivision === 0.5) confidence *= 0.7; // Half bar (usually too short)
...
function selectBestLoop(candidates, audioData, sampleRate) {
  ...
  console.log(`🏆 Best loop: ${best.start.toFixed(3)}s - ${best.end.toFixed(3)}s`);
  console.log(`🎵 ${best.musicalDivision} bars, confidence: ${best.confidence.toFixed(3)}`);
...
function snapToNearestBeat(loop, beats, tempo) {
  ...
  if (nearestStartBeat !== loop.start || nearestEndBeat !== loop.end) {
    console.log(`🔧 Phase correction: moved start ${(nearestStartBeat - loop.start).toFixed(3)}s, end ${(nearestEndBeat - loop.end).toFixed(3)}s`);
  }
