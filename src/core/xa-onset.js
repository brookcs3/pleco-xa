  ...
  for (let len = 2; len <= N; len <<= 1) {
    ...
    for (let i = 0; i < N; i += len) {
      ...
      for (let j = 0; j < len / 2; j++) {
        ...
        const v = [
          reversed[(i + j + len / 2) * 2] * w[0] - reversed[(i + j + len / 2) * 2 + 1] * w[1],
          reversed[(i + j + len / 2) * 2] * w[1] + reversed[(i + j + len / 2) * 2 + 1] * w[0]
        ];
 ...
 */
export function onsetDetect(audioData, sampleRate, {
  hopLength = 512,
  frameLength = 2048,
  delta = 0.07,
  wait = 20
  ...
  const onsetTimes = onsetFrames.map(frame => frame * hopLength / sampleRate);
...
export function computeSpectralFlux(stft) {
  ...
  for (let i = 1; i < stft.length; i++) {
    ...
    for (let j = 0; j < currentFrame.length; j += 2) {
      const currentMag = Math.sqrt(currentFrame[j] ** 2 + currentFrame[j + 1] ** 2);
...
export function pickPeaks(onsetStrength, { delta = 0.07, wait = 20 } = {}) {
  ...
  // Adaptive threshold
  const meanStrength = onsetStrength.reduce((a, b) => a + b, 0) / onsetStrength.length;
