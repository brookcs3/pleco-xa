...
export function findKickSnareHit(audioData, sampleRate, loop) {
  ...
  const startSample = Math.floor(Math.max(0, (loop.start - searchWindow * 0.5) * sampleRate));
  const endSample = Math.floor(Math.min(audioData.length, (loop.start + searchWindow) * sampleRate));
  ...
  for (let pos = startSample; pos < endSample - windowSize; pos += hopSize) {
    ...
    const freqScore = checkFrequencySpread(audioData.slice(pos, pos + windowSize));
  ...
  if (maxTransientScore > 5.0 && Math.abs(detectedTime - loop.start) > 0.01) {
    ...
    console.log(`🥁 Found kick+snare at ${detectedTime.toFixed(3)}s (score: ${maxTransientScore.toFixed(2)})`);
    console.log(`🥁 Adjusted to ${adjustedTime.toFixed(3)}s (+20ms to hit beat center)`);
  ...
  console.log(`🥁 No clear kick+snare found (max score: ${maxTransientScore.toFixed(2)})`);
  return null;
}
