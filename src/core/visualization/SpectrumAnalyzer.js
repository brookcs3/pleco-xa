...
export async function renderStaticSpectrum(canvas, audioBuffer, options = {}) {
  ...
  const tempAnalyzer = new RealtimeSpectrumAnalyzer(canvas, { sampleRate: audioBuffer.sampleRate }, opts);
  ...
  const peakFrequency = (peakIndex / frequencyData.length) * (audioBuffer.sampleRate / 2);
  
  const spectralCentroid = calculateSpectralCentroid(frequencyData, audioBuffer.sampleRate);
  const spectralRolloff = calculateSpectralRolloff(frequencyData, audioBuffer.sampleRate);
...
export async function createSpectrogram(canvas, audioBuffer, options = {}) {
  ...
  const numFrames = Math.floor((channelData.length - opts.fftSize) / opts.hopLength) + 1;
  ...
  for (let frame = 0; frame < numFrames; frame++) {
    ...
    for (let i = 0; i < frameData.length; i++) {
      const windowValue = 0.5 * (1 - Math.cos(2 * Math.PI * i / (frameData.length - 1)));
  ...
  for (let x = 0; x < width; x++) {
    ...
    for (let y = 0; y < height; y++) {
      ...
      const color = getSpectrogramColor(Math.max(0, Math.min(1, normalized)), opts.colormap);
...
function getSpectrogramColor(value, colormap) {
  ...
  switch (colormap) {
    ...
    case 'viridis':
      const r = Math.floor(255 * (0.267 + 0.105 * value + 0.547 * value * value));
      const g = Math.floor(255 * (0.005 + 0.628 * value + 0.367 * value * value));
