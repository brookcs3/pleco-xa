...
export function renderWaveform(canvas, waveformData, options = {}) {
  ...
  switch (opts.style) {
    ...
    case 'line':
      ...
      break;
    case 'filled':
      renderFilled(ctx, waveformData, width, height, centerY, maxAmplitude, opts);
      break;
    default:
...
export function createInteractiveRenderer(canvas, options = {}) {
  ...
  function handleMouseMove(e) {
    ...
    if (isSelecting) {
      ...
      emit('selectionChange', Math.min(selectionStart, selectionEnd), Math.max(selectionStart, selectionEnd));
    }
  ...
  function handleMouseUp(e) {
    ...
    if (Math.abs(end - start) > 0.01) { // Minimum selection threshold
  ...
  function render(waveformData, renderOptions = {}) {
    ...
    if (opts.enableSelection && (isSelecting || renderOptions.preserveSelection)) {
      const ctx = canvas.getContext('2d');
      const startX = (Math.min(selectionStart, selectionEnd) / duration) * canvas.width;
      const endX = (Math.max(selectionStart, selectionEnd) / duration) * canvas.width;
