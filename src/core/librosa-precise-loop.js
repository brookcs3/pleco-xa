...
export function findPreciseLoop(audioData, sampleRate, tempo, {
  ...
  console.log(`🔍 Searching for loops between ${minLoopDuration}s and ${maxLoopDuration}s`);
  ...
  for (let i = 0; i < onsets.length; i++) {
    ...
    for (let j = i + 1; j < onsets.length; j++) {
      ...
      if (totalScore > bestScore) {
        ...
        console.log(`✨ New best loop: ${startTime.toFixed(3)}s - ${endTime.toFixed(3)}s (${loopDuration.toFixed(3)}s), score: ${totalScore.toFixed(4)}`);
      }
    }
  }
...
function fineTuneLoopStart(audioData, sampleRate, loop, onsets) {
  ...
  console.log(`🔧 Fine-tuning: found ${nearbyOnsets.length} onsets near ${loop.start.toFixed(3)}s`);
  ...
  if (bestStart !== loop.start) {
    console.log(`✨ Adjusted loop start: ${loop.start.toFixed(3)}s -> ${bestStart.toFixed(3)}s`);
...
function findNextStrongAttack(audioData, sampleRate, loop) {
  ...
  if (maxEnergyOffset > 0) {
    ...
    console.log(`🎯 Found stronger attack ${(maxEnergyOffset / sampleRate * 1000).toFixed(1)}ms later`);
