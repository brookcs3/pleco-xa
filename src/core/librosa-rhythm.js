 ...
 */
export function predominantLocalPulse(onsetStrength, tempo, sampleRate, hopLength = 512) {
 ...
 */
export function onsetStrengthMulti(audioData, sampleRate, {
  hopLength = 512,
  frameLength = 2048,
  nMels = 128,
  fMin = 80,
  ...
  const melFilters = createMelFilterbank(sampleRate, frameLength, nMels, fMin, fMax);
 ...
 */
export function viterbiBeats(onsetStrength, tempo, sampleRate, hopLength = 512, tightness = 100) {
  ...
  const dp = Array(nFrames).fill(null).map(() => new Float32Array(nStates).fill(-Infinity));
  const path = Array(nFrames).fill(null).map(() => new Int32Array(nStates));
  ...
  for (let frame = 1; frame < nFrames; frame++) {
    for (let state = 0; state < nStates; state++) {
      ...
      for (let prevState = 0; prevState < nStates; prevState++) {
        ...
        const penalty = Math.pow((interval - expectedInterval) / expectedInterval, 2) * tightness;
  ...
  for (let frame = nFrames - 1; frame >= 0; frame--) {
    if (frame % Math.round(beatPeriod) === Math.round(phaseOffset) % Math.round(beatPeriod)) {
...
function findDownbeatsFromBeats(audioData, beats, sampleRate) {
  ...
  for (let i = 0; i < beats.length; i++) {
    ...
    const change = getSpectralChange(audioData, prevBeatSample, beatSample, frameSize);
