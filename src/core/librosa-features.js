 ...
 */
export function spectral_bandwidth(y, sr = 22050, hop_length = 512, n_fft = 2048, p = 2) {
    const { stft, magnitude, fft_frequencies } = require('./librosa-fft.js');
    ...
    for (let t = 0; t < mag_spec.length; t++) {
        ...
        const bandwidth = magnitude_sum > 0 ? 
            Math.pow(weighted_deviation / magnitude_sum, 1/p) : 0;
 ...
 */
export function spectral_rolloff(y, sr = 22050, hop_length = 512, n_fft = 2048, roll_percent = 0.85) {
    const { stft, magnitude, fft_frequencies } = require('./librosa-fft.js');
 ...
 */
export function spectral_contrast(y, sr = 22050, hop_length = 512, n_fft = 2048, n_bands = 6) {
    ...
    const contrast = Array(n_bands).fill(null).map(() => new Float32Array(n_frames));
    ...
    for (let t = 0; t < n_frames; t++) {
        for (let b = 0; b < n_bands; b++) {
            ...
            if (start_bin < end_bin) {
                ...
                contrast[b][t] = peak > 0 ? Math.log(Math.max(peak, 1e-10) / Math.max(valley, 1e-10)) : 0;
            }
        }
    }
...
export function normalize_features(features, norm = 'l2', axis = 0) {
    ...
    if (norm === 'l2') {
        ...
        if (axis === 0) {
            ...
            const normalized = Array(n_features).fill(null).map(() => new Float32Array(n_frames));
            for (let t = 0; t < n_frames; t++) {
                let sum_sq = 0;
...
export function compute_feature_stats(features) {
    ...
    for (let f = 0; f < n_features; f++) {
        ...
        // Standard deviation
        const variance = feature.reduce((acc, val) => acc + (val - mean) ** 2, 0) / n_frames;
        ...
        const skew = feature.reduce((acc, val) => 
            acc + Math.pow((val - mean) / stats.std[f], 3), 0) / n_frames;
        stats.skewness[f] = stats.std[f] > 0 ? skew : 0;
        
        // Kurtosis (fourth moment)
        const kurt = feature.reduce((acc, val) => 
            acc + Math.pow((val - mean) / stats.std[f], 4), 0) / n_frames;
