...
export function smartLoopDetect(y, sr, hop = 512, maxBars = 8) {
  ...
  let bestStart = 0, bestLen = 0, bestLag = 0;
  for (let lag = 1; lag < Lenh.length; ++lag) {
    ...
    for (let i = 0; i < col.length; ++i) {
      if (col[i] > 0) {
        runStart ??= i; runLen++;
        if (runLen > bestLen) { bestLen = runLen; bestStart = runStart; bestLag = lag; }
  ...
  const nearest    = t => beatTimes.reduce((a,b) => Math.abs(b-t)<Math.abs(a-t)?b:a);
  ...
  const [trimStart, trimEnd] = DynamicZeroCrossing.snap(
      y, Math.round(startSec*sr), Math.round(endSec*sr));
