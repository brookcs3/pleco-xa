 ...
 */
export async function example(key, hq = false, baseUrl = 'https://librosa.org/data/audio/') {
    // Validate example key
    if (!AUDIO_REGISTRY[key]) {
        const available = Object.keys(AUDIO_REGISTRY);
        throw new FileError(`Unknown example key: ${key}. Available: ${available.join(', ')}`);
    }
    ...
    try {
        ...
        console.log(`Loading example: ${key} (${hq ? 'HQ' : 'standard'}) from ${baseUrl}${filename}`);
        ...
        if (!response.ok) {
            throw new FileError(`Failed to fetch ${filename}: ${response.status} ${response.statusText}`);
        }
 ...
 */
export async function exampleBuffer(key, hq = false, audioContext = null, baseUrl = 'https://librosa.org/data/audio/') {
    if (!audioContext) {
 ...
 */
export async function exampleAudio(key, hq = false, channel = 0, audioContext = null) {
    const buffer = await exampleBuffer(key, hq, audioContext);
    
    if (channel >= buffer.numberOfChannels) {
        throw new FileError(`Channel ${channel} not available. Audio has ${buffer.numberOfChannels} channels.`);
    }
 ...
 */
export function saveAudio(audioData, sampleRate, filename = 'audio.wav', format = 'wav') {
    if (format !== 'wav') {
...
export function cache() {
    return {
        ...
        preload: async (keys = Object.keys(AUDIO_REGISTRY), hq = false) => {
            const promises = keys.map(key => example(key, hq).catch(err => {
