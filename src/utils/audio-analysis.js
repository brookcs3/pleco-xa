...
async function loadSampleFile(url, name) {
  try {
    ...
    console.log(`✅ Audio loaded: ${currentAudioBuffer.duration.toFixed(2)}s @ ${currentAudioBuffer.sampleRate}Hz`);
...
async function detectLoop() {
  try {
    ...
    if (endSec <= startSec || !Number.isFinite(startSec) || !Number.isFinite(endSec)) {
      console.warn('fastLoopAnalysis gave unusable bounds; reverting to 4‑bar guess');
    ...
    try {
      ...
      if (maxIdx > hop) { // ignore first tiny blips
        ...
        console.log(`🎯 Start nudged to onset @ ${(startSample / sr).toFixed(3)}s`);
      }
    } catch (e) {
...
async function playAudio() {
  ...
  try {
    ...
    console.log(`🎮 Audio context destination: ${audioContext.destination.channelCount} channels`);
