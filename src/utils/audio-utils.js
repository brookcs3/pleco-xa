...
export async function analyzeWithReference(audioBuffer, template) {
  ...
  debugLog(
    `Reference analysis: scanning ${totalSamples} samples with template of ${templateLength} samples`,
  )
  ...
  const endSample = findZeroCrossing(
    channelData,
    bestMatch.position + templateLength,
  )
